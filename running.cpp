...
enum State { FOLLOW_LINE, BALL_SEEN, APPROACH_BALL, INTAKE, RELOCK_LINE };
...
int clamp255(float x){ if (x>1) x=1; if (x<-1) x=-1; return int(std::round(x*255)); }
void sendMotorCmd(int fd, const char* id, int spd){ char buf[32]; int n=snprintf(buf,sizeof(buf),"%s %d\n",id,spd); write(fd,buf,n); }
void driveLR(int fd, float vL, float vR){ sendMotorCmd(fd,"M1",clamp255(vL)); sendMotorCmd(fd,"M2",clamp255(vR)); }
...
int main(){
    ...
    if (uart<0) { perror("UART"); /* vẫn cho chạy demo hình ảnh */ }
    ...
    while (true){
        ...
        rs2::frameset fs;
        if (!pipe.poll_for_frames(&fs)) { std::this_thread::sleep_until(next_tick); continue; }

        rs2::video_frame color = fs.get_color_frame();
        rs2::depth_frame depth = fs.get_depth_frame();
        if (!color) { std::this_thread::sleep_until(next_tick); continue; }
        ...
        for (int y=ROI_TOP; y<HEIGHT; ++y){
            const Vec3b* row = bgr.ptr<Vec3b>(y); uchar* mrow = mask.ptr<uchar>(y);
            for (int x=0; x<WIDTH; ++x){
                int B=row[x][0], G=row[x][1], R=row[x][2], V=std::max({B,G,R});
                if (B>G+T && B>R+T && V>=Vmin) mrow[x]=255;
            }
        }
        morphologyEx(mask, mask, MORPH_CLOSE, getStructuringElement(MORPH_RECT, {5,5}), Point(-1,-1), 2);

        // ---- Find centroid of largest contour (line) ----
        std::vector<std::vector<Point>> contours; findContours(mask, contours, RETR_EXTERNAL, CHAIN_APPROX_SIMPLE);
        int centerX = WIDTH/2; int cx_line = -1;
        if (!contours.empty()){
            auto cmax = *std::max_element(contours.begin(), contours.end(), [](auto&a,auto&b){return contourArea(a)<contourArea(b);});
            Moments M = moments(cmax);
            if (M.m00 >= 1e-3) cx_line = int(M.m10/M.m00);
        }
        ...
        switch(st){
            case FOLLOW_LINE:{
                if (cx_line>=0){
                    ...
                    line_ok_hold = 0;
                    vL=-0.18f; vR=0.18f; // quay tìm line
                }
                // thấy bóng đủ tốt?
                if (r.valid && r.score>Smin && r.dist_m < D_detect_max){
                    if (++seen_hold >= N_detect_hold) { st=BALL_SEEN; }
                } else seen_hold=0;
                break;
            }
            case BALL_SEEN:{
            ...
            case APPROACH_BALL:{
                if (r.valid){
                    ...
                    if (uart>=0 && r.dist_m < D_intake_on) intake(uart, +INTAKE_PWM);
                    if (r.dist_m < D_captured){ st=INTAKE; }
                } else {
            ...
            case INTAKE:{
                if (uart>=0) intake(uart, +INTAKE_PWM);
            ...
            case RELOCK_LINE:{
                if (cx_line>=0){
                    auto [l,r] = follower.update(cx_line, centerX);
                    vL=l; vR=r;
                    if (++line_ok_hold >= LINE_OK_HOLD){ st=FOLLOW_LINE; line_ok_hold=0; }
                } else {
                    line_ok_hold=0;
                    vL=-0.18f; vR=0.18f; // quay tìm line
                }
                // tắt intake khi đã bắt đầu relock
                if (uart>=0) intake(uart, 0);
                break;
            }
        }

        // ---- Gửi motor xuống Arduino ----
        if (uart>=0) { driveLR(uart, vL, vR); }

        // ---- Overlay debug ----
        line(bgr, {WIDTH/2,0},{WIDTH/2,HEIGHT-1},{255,255,0},1,LINE_AA);
        line(bgr, {0,ROI_TOP},{WIDTH-1,ROI_TOP},{200,200,200},1,LINE_AA);
        if (cx_line>=0){ line(bgr, {cx_line,ROI_TOP},{cx_line,HEIGHT-1},{0,255,255},2,LINE_AA); }
        char txt[128];
    ...
    if (uart>=0) { driveLR(uart,0,0); intake(uart,0); close(uart); }
    pipe.stop();
    return 0;
}
